...
resource "aws_nat_gateway" "eks_gw_1a" {
...
resource "aws_nat_gateway" "eks_gw_1b" {
...
resource "aws_route_table" "eks_private_route_Table_1a" {
  ...
  route {
    ...
    nat_gateway_id = aws_nat_gateway.eks_gw_1a.id
  }
...
resource "aws_route_table" "eks_private_route_Table_1b" {
  ...
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.eks_gw_1b.id
  }
